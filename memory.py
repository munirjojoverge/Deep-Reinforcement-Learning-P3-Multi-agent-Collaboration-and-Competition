...
class ReplayMemory():
  def __init__(self, args):
    ...
    self.num_memories = 0  # Internal episode timestep counter
    ...
    self.priority_weight_increase = (1 - args.priority_weight) / (args.T_max - int(self.capacity * args.learning_starts_ratio))
  ...
  def add(self, state, action, reward, terminal):
    ...
    self.transitions.add(Transition(self.num_memories, state, action, reward, terminal), self.transitions.max)  # Store new transition with maximum priority
    self.num_memories = 0 if terminal else self.num_memories + 1  # Start new episodes with t = 0
    self.priority_weight = min(self.priority_weight + self.priority_weight_increase, 1)  # Anneal importance sampling weight β to 1
